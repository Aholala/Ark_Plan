...
void NRF24L01_W_CE(uint8_t BitValue)
{
	...
	HAL_GPIO_WritePin(NRF_CE_GPIO_Port, NRF_CE_Pin, BitValue ? GPIO_PIN_SET : GPIO_PIN_RESET);
}
...
void NRF24L01_W_CSN(uint8_t BitValue)
{
	...
	HAL_GPIO_WritePin(NRF_CSN_GPIO_Port, NRF_CSN_Pin, BitValue ? GPIO_PIN_SET : GPIO_PIN_RESET);
}
...
void NRF24L01_W_SCK(uint8_t BitValue)
{
	...
	HAL_GPIO_WritePin(NRF_SCK_GPIO_Port, NRF_SCK_Pin, BitValue ? GPIO_PIN_SET : GPIO_PIN_RESET);
}
...
void NRF24L01_W_MOSI(uint8_t BitValue)
{
	...
	HAL_GPIO_WritePin(NRF_MOSI_GPIO_Port, NRF_MOSI_Pin, BitValue ? GPIO_PIN_SET : GPIO_PIN_RESET);
}
...
{
	/*取MISO引脚的高低电平并返回*/
	return (HAL_GPIO_ReadPin(NRF_MISO_GPIO_Port, NRF_MISO_Pin) == GPIO_PIN_SET) ? 1 : 0;
}
...
void NRF24L01_Init(void)
{
	...
	NRF24L01_WriteReg(NRF24L01_SETUP_RETR, 0x2F);	//设置自动重传，间隔750us，重传15次
