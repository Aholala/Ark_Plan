 ...
 * 数据包宽度固定为 NRF24L01_PACKET_WIDTH（通常在源文件中定义为32字节）。
 */
...
/**
 * @brief 发送数据包缓冲区
 * @note  长度为 NRF24L01_PACKET_WIDTH（通常为32字节）
 *        发送前将待发送数据填入该缓冲区，然后调用 NRF24L01_Send()
 */
 ...
 * @brief 接收数据包缓冲区
 * @note  长度为 NRF24L01_PACKET_WIDTH（通常为32字节）
 *        调用 NRF24L01_Receive() 成功接收后，数据存放于此
 */
...
/**
 * @brief 读单个寄存器
 * @param RegAddress 寄存器地址（如 NRF24L01_CONFIG）
 * @return uint8_t 寄存器值
 */
